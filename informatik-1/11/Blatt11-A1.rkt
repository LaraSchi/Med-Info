...
(define btree-fold
  (lambda (z c t)
    (match t
      ((make-empty-tree) z)
      ((make-node l x r) (c (btree-fold z c l)
                            x
                            (btree-fold z c r))))))
;T1
;    1    
...
(: t1 (btree-of integer))
(define t1
  (make-node the-empty-tree 1 (make-node (make-node the-empty-tree -5 the-empty-tree) 0 (make-node the-empty-tree 3 the-empty-tree))))
;T2
;    0    
;┌───┴┐   
;▢    9   
...
(: t2 (btree-of integer))
(define t2
  (make-node the-empty-tree 0 (make-node (make-node the-empty-tree 0 the-empty-tree) 9 (make-node the-empty-tree -5 the-empty-tree))))

;Min node of btree
(: btree-min ((btree-of real) -> real))
(check-within (btree-min t1) -5.0 0.001)
(check-within (btree-min t2) -5.0 0.001)
(define btree-min
  (lambda (bt) 
...
(check-within (btree-max t1) 3 0.001)
(check-within (btree-max t2) 9 0.001)
(define btree-max
  (lambda (bt) 
