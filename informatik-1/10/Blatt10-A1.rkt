...
#reader(lib "DMdA-vanilla-reader.ss" "deinprogramm")((modname Blatt10-A1) (read-case-sensitive #f) (teachpacks ()) (deinprogramm-settings #(#f write repeating-decimal #t #t none explicit #f ())))
; ----------------------------------------------------------------------
...
;Kreiert einen konstanten Strom
(: const-stream (%a -> (stream-of %a)))
(check-expect (stream-take 3 (const-stream 1)) (list 1 1 1))
(check-expect (stream-take 3 (const-stream "A")) (list "A" "A" "A"))
(define const-stream
  (lambda (a)
    (make-cons a (lambda () (const-stream a)))
    ))
;Strom von einsen
(: ones (stream-of (one-of 1)))
(define ones
  (const-stream 1))

;Wendet prozedur auf jeden Kof des Stroms an
(: stream-map ((%a -> %b) (stream-of %a) -> (stream-of %b)))
(check-expect (stream-take 3 (stream-map (lambda (x) (+ x 1)) ones)) (list 2 2 2))
(check-expect (stream-take 3 (stream-map (lambda (x) 2) (const-stream "A"))) (list 2 2 2))
(define stream-map
  (lambda (p s)
    (make-cons (p (head s)) (lambda () (stream-map p (force (tail s)))))))
